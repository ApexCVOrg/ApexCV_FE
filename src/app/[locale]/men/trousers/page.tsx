...
export default function TrousersPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/products?gender=men&category=trousers`);
        if (!response.ok) {
  ...
  const refreshProducts = async () => {
    ...
    try {
      ...
      console.error('[TrousersPage] Error:', err);
      setError(err instanceof Error ? err.message : 'Failed to fetch products');
    } finally {
      setLoading(false);
    }
    ...
    <>
      ...
      <Box sx={{ px: { xs: 2, md: 6 }, pt: 4, pb: 2 }}>
        ...
        <Typography variant="body1" sx={{ mt: 2, maxWidth: 900, color: 'text.secondary', fontSize: { xs: '1rem', md: '1.1rem' } }}>
          Explore our men's trousers collection – from casual joggers to classic pants, designed for comfort and versatility. Perfect for sports, leisure, or everyday wear.
        </Typography>
      </Box>
