...
export default function TracksuitPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/products?gender=men&category=tracksuit`);
        if (!response.ok) {
    ...
    <>
      ...
      <Box sx={{ px: { xs: 2, md: 6 }, pt: 4, pb: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 1 }}>
          ...
          <Typography component="span" sx={{ color: 'grey.400', mx: 0.5 }}>/</Typography>
          <Typography component="span" sx={{ color: 'text.primary', fontWeight: 500, textDecoration: 'underline', textUnderlineOffset: '4px', fontSize: '1rem' }}>Tracksuit</Typography>
        </Box>
        ...
        <Typography variant="body1" sx={{ mt: 2, maxWidth: 900, color: 'text.secondary', fontSize: { xs: '1rem', md: '1.1rem' } }}>
          Discover our collection of men's tracksuits – perfect for training, lounging, or casual wear. Comfortable, stylish, and designed for performance.
        </Typography>
      </Box>
