...
export default function JacketPage() {
  ...
  const refreshProducts = async () => {
    ...
    try {
      ...
      console.error('[JacketPage] Error:', err);
      setError(err instanceof Error ? err.message : 'Failed to fetch products');
    } finally {
      setLoading(false);
    }
    ...
    <>
      ...
      <Box sx={{ px: { xs: 2, md: 6 }, pt: 4, pb: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 1 }}>
          ...
          <Typography component="span" sx={{ color: 'grey.400', mx: 0.5 }}>/</Typography>
          <Typography component="span" sx={{ color: 'text.primary', fontWeight: 500, textDecoration: 'underline', textUnderlineOffset: '4px', fontSize: '1rem' }}>Jackets</Typography>
        </Box>
        ...
        <Typography variant="body1" sx={{ mt: 2, maxWidth: 900, color: 'text.secondary', fontSize: { xs: '1rem', md: '1.1rem' } }}>
          Stay protected and stylish with our men's jackets – from lightweight windbreakers to insulated coats. Designed for all weather, every adventure, and everyday comfort.
        </Typography>
      </Box>
