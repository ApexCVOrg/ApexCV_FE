...
export default function ThunPoloPage() {
  ...
  const sortProducts = (products: Product[], sortType: string) => {
    ...
    switch (sortType) {
      ...
      case 'newest':
        return sortedProducts; // giữ nguyên thứ tự nếu không có createdAt
      case 'popular':
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/products?gender=men&category=thunpolo`);
        if (!response.ok) {
  ...
  const refreshProducts = async () => {
    ...
    try {
      ...
      console.error('[ThunPoloPage] Error:', err);
      setError(err instanceof Error ? err.message : 'Failed to fetch products');
    } finally {
      setLoading(false);
    }
    ...
    <>
      ...
      <Box sx={{ px: { xs: 2, md: 6 }, pt: 4, pb: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 1 }}>
          ...
          <Typography component="span" sx={{ color: 'grey.400', mx: 0.5 }}>/</Typography>
          <Typography component="span" sx={{ color: 'text.primary', fontWeight: 500, textDecoration: 'underline', textUnderlineOffset: '4px', fontSize: '1rem' }}>T-shirts</Typography>
        </Box>
