...
export default function HoodiePage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/products?gender=men&category=hoodie`);
        if (!response.ok) {
    ...
    <>
      ...
      <Box sx={{ px: { xs: 2, md: 6 }, pt: 4, pb: 2 }}>
        ...
        <Typography variant="body1" sx={{ mt: 2, maxWidth: 900, color: 'text.secondary', fontSize: { xs: '1rem', md: '1.1rem' } }}>
          Discover our range of men's hoodies – from classic pullovers to modern zip-ups, perfect for layering and keeping warm in style. Soft fabrics and athletic fits for every occasion.
        </Typography>
      </Box>
