...
export default function ProductsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const submitData = {
        ...formData,
        categories: formData.categories.map(cat => (typeof cat === 'string' ? cat : cat._id || cat)),
              ...
              <FormControl fullWidth required sx={{
                ...
                <Select
                  ...
                  onChange={e => {
                    ...
                    setFormData(prev => ({
                      ...prev,
                      categories: categories.map(cat => typeof cat === 'string' ? cat : cat._id || ''),
                    }));
                  }}
                  label="Categories"
                  sx={{
                  ...
                  renderValue={selected => (
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                      {selected.map((value, idx) => {
                        const categoryId = typeof value === 'string' ? value : value._id || '';
