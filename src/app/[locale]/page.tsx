...
export default function HomePage() {
  ...
  return (
    <>
                  ...
                  {products['filtered']?.slice(0, visibleCount).length > 0 ? products['filtered'].slice(0, visibleCount).map((product) => {
                    ...
                    return (
                      <ProductCard
                        ...
                        labels={PRODUCT_LABELS.map(l => l.value).includes(product.label as ProductLabel) ? [product.label as ProductLabel] : []}
                    ...
                    return (
                      <Box key={product._id} sx={{ minWidth: 320, flex: '0 0 auto' }}>
                        <ProductCard
                          ...
                          allCategories={categories}
                          allBrands={brands}
                          onAddToCart={() => setSelectedProductId(product._id)}
                          backgroundColor="#f8f9fa"
                          colors={3}
                        />
                      </Box>
      ...
      {/* Sidebar chi tiết sản phẩm luôn hiển thị bên phải */}    </>
  );
}