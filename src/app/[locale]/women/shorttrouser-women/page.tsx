    ...
    try {
      ...
      const filtered = (data.data || []).filter((item: any) => {
        ...
        if (Array.isArray(item.categoryPath)) {
          const hasShorts = item.categoryPath.some((cat: string) => 
        ...
        if (item.categories && Array.isArray(item.categories)) {
          ...
          const hasShortsCategory = categoryNames.some((name: string) => 
        ...
        if (item.tags && Array.isArray(item.tags)) {
          const hasShortsTag = item.tags.some((tag: string) => 
        ...
        // Kiểm tra trong name
        if (item.name.toLowerCase().includes('shorts') || item.name.toLowerCase().includes('short')) return true;
