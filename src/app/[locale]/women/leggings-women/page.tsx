...
export default function WomenLeggingsPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const allWomenProducts = (data.data || []).filter(
          (p: any) => p.categories?.[1] && teamNames.includes(p.categories[1].name.toLowerCase())
        );
        const sorted = sortProducts(allWomenProducts, sortBy);
        setProducts(sorted);
    ...
    <div style={{ padding: 32 }}>
      <Box sx={{ px: { xs: 2, md: 6 }, pt: 4, pb: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 1 }}>
          ...
          <Typography component="span" sx={{ color: 'grey.400', mx: 0.5 }}>/</Typography>
          <Typography component="span" sx={{ color: 'text.primary', fontWeight: 500, textDecoration: 'underline', textUnderlineOffset: '4px', fontSize: '1rem' }}>Leggings</Typography>
        </Box>
        ...
        <Typography variant="body1" sx={{ mt: 2, maxWidth: 900, color: 'text.secondary', fontSize: { xs: '1rem', md: '1.1rem' } }}>
          Thoải mái vận động với bộ sưu tập quần leggings nữ, chất liệu co giãn tốt, thoáng khí, phù hợp cho yoga, gym và các hoạt động thể thao khác.
        </Typography>
      </Box>
      ...
      {products.length === 0 && <p>Không có sản phẩm quần leggings nào.</p>}
      <Box
        sx={{
          display: 'grid',
          gridTemplateColumns: {
