...
export default function WomenDressPage() {
  ...
  const sortProducts = (products: Product[], sortType: string) => {
    console.log('[DressPage] Sorting products by:', sortType);
  ...
  useEffect(() => {
    const fetchDresses = async () => {
      ...
      try {
        ...
        console.log('[DressPage] Fetching products with URL:', apiUrl);
        ...
        const dresses = data.data || [];

        console.log('[DressPage] Filtered dresses before sorting:', dresses.length);
        console.log('[DressPage] Current sortBy value:', sortBy);
        
        const sortedDresses = sortProducts(dresses, sortBy);
        console.log('[DressPage] Final sorted dresses count:', sortedDresses.length);
        
        setProducts(sortedDresses);
      } catch (err) {
        console.error('[DressPage] Error:', err);
    ...
    <>
      ...
      <Box sx={{ px: { xs: 2, md: 6 }, pt: 4, pb: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 1 }}>
          ...
          <Typography component="span" sx={{ color: 'grey.400', mx: 0.5 }}>/</Typography>
          <Typography component="span" sx={{ color: 'text.primary', fontWeight: 500, textDecoration: 'underline', textUnderlineOffset: '4px', fontSize: '1rem' }}>Dresses</Typography>
        </Box>
        ...
        <Typography variant="body1" sx={{ mt: 2, maxWidth: 900, color: 'text.secondary', fontSize: { xs: '1rem', md: '1.1rem' } }}>
          Discover our collection of women's dresses, perfect for any occasion. From casual to elegant, find your perfect style.
        </Typography>
      </Box>
      {/* Sort Bar */}
