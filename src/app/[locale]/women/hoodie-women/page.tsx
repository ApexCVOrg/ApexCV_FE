...
export default function WomenHoodiePage() {
  ...
  const sortProducts = (products: Product[], sortType: string) => {
    console.log('[HoodiePage] Sorting products by:', sortType);
  ...
  useEffect(() => {
    const fetchHoodies = async () => {
      ...
      try {
        ...
        console.log('[HoodiePage] Fetching products with URL:', apiUrl);
        ...
        const hoodies = data.data || [];

        console.log('[HoodiePage] Filtered hoodies before sorting:', hoodies.length);
        console.log('[HoodiePage] Current sortBy value:', sortBy);
        
        const sortedHoodies = sortProducts(hoodies, sortBy);
        console.log('[HoodiePage] Final sorted hoodies count:', sortedHoodies.length);
        
        setProducts(sortedHoodies);
      } catch (err) {
        console.error('[HoodiePage] Error:', err);
    ...
    <>
      ...
      <Box sx={{ px: { xs: 2, md: 6 }, pt: 4, pb: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 1 }}>
          ...
          <Typography component="span" sx={{ color: 'grey.400', mx: 0.5 }}>/</Typography>
          <Typography component="span" sx={{ color: 'text.primary', fontWeight: 500, textDecoration: 'underline', textUnderlineOffset: '4px', fontSize: '1rem' }}>Hoodies</Typography>
        </Box>
        ...
        <Typography variant="body1" sx={{ mt: 2, maxWidth: 900, color: 'text.secondary', fontSize: { xs: '1rem', md: '1.1rem' } }}>
          Explore our collection of women's hoodies, perfect for comfort and style. From casual to sporty, find your perfect hoodie.
        </Typography>
      </Box>
      {/* Sort Bar */}
          ...
          <>
            {products.length === 0 && (
              <Typography variant="body1" sx={{ textAlign: 'center', py: 4 }}>
                Không có sản phẩm áo hoodie nào.
              </Typography>
            )}
            <Box
              sx={{
