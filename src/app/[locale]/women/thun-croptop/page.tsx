...
export default function WomenCropTopPage() {
  ...
  const sortProducts = (products: Product[], sortType: string) => {
    console.log('[CropTopPage] Sorting products by:', sortType);
  ...
  useEffect(() => {
    const fetchCropTops = async () => {
      ...
      try {
        ...
        console.log('[CropTopPage] Fetching products with URL:', apiUrl);
        ...
        const cropTops = data.data || [];

        console.log('[CropTopPage] Filtered crop tops before sorting:', cropTops.length);
        console.log('[CropTopPage] Current sortBy value:', sortBy);
        
        const sortedCropTops = sortProducts(cropTops, sortBy);
        console.log('[CropTopPage] Final sorted crop tops count:', sortedCropTops.length);
        
        setProducts(sortedCropTops);
      } catch (err) {
        console.error('[CropTopPage] Error:', err);
    ...
    <>
      ...
      <Box sx={{ px: { xs: 2, md: 6 }, pt: 4, pb: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 1 }}>
          ...
          <Typography component="span" sx={{ color: 'grey.400', mx: 0.5 }}>/</Typography>
          <Typography component="span" sx={{ color: 'text.primary', fontWeight: 500, textDecoration: 'underline', textUnderlineOffset: '4px', fontSize: '1rem' }}>Crop Tops</Typography>
        </Box>
        ...
        <Typography variant="body1" sx={{ mt: 2, maxWidth: 900, color: 'text.secondary', fontSize: { xs: '1rem', md: '1.1rem' } }}>
          Discover our collection of women's crop tops, perfect for a trendy and stylish look. From casual to sporty, find your perfect crop top.
        </Typography>
      </Box>
      {/* Sort Bar */}
          ...
          <>
            {products.length === 0 && (
              <Typography variant="body1" sx={{ textAlign: 'center', py: 4 }}>
                Không có sản phẩm áo crop top nào.
              </Typography>
            )}
            <Box
              sx={{
