...
const superstarProducts = [
  ...
  },
  {
    ...
    price: "$135",
    image: "/assets/images/shoes/superstar/Giay_Superstar_Vintage_trang_JQ3254_02_standard.avif",
    rating: 4.8,
    reviews: 267
  },
  {
    ...
    price: "$150",
    image: "/assets/images/shoes/superstar/Giay_Superstar_Vintage_trang_JQ3254_03_standard.avif",
    rating: 4.9,
    reviews: 289
  }
];
      ...
      <Box sx={{ position: 'relative', zIndex: 2 }}>
        ...
        <Box sx={{ px: { xs: 2, md: 6 }, pt: 4, pb: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 1 }}>
            ...
            <Typography component="span" sx={{ color: 'text.primary', fontWeight: 500, textDecoration: 'underline', textUnderlineOffset: '4px', fontSize: '1rem' }}>Superstar</Typography>
          </Box>
          ...
          <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-start', alignItems: 'flex-end', pb: 2 }}>
            {TABS.map(tab => (
              <Link key={tab.value} href={`/shoes/${tab.value}`} style={{ textDecoration: 'none', flex: 1 }}>
                <Box sx={{
                  border: 0,
                  borderBottom: tab.value === 'superstar' ? '4px solid #000' : 'none',
                  bgcolor: tab.value === 'superstar' ? '#000' : '#fff',
                  color: tab.value === 'superstar' ? '#fff' : '#111',
                  fontWeight: tab.value === 'superstar' ? 700 : 500,
                  textAlign: 'center',
                  cursor: 'pointer',
                  transition: 'all 0.2s',
                  p: 0,
                  ...
                  '&:hover': {
                    ...
                    bgcolor: tab.value === 'superstar' ? '#000' : '#f5f5f5',
                    color: '#000',
                    fontWeight: 700,
                  ...
                  <Typography sx={{ py: 2, fontWeight: tab.value === 'superstar' ? 700 : 600, fontSize: '1.1rem', letterSpacing: 1, bgcolor: tab.value === 'superstar' ? '#000' : '#fff', color: tab.value === 'superstar' ? '#fff' : '#111', textTransform: 'uppercase' }}>{tab.label}</Typography>
                </Box>
              </Link>
            ))}
          </Box>
