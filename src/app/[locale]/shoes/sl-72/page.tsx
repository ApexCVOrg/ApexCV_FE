...
export default function SL72Page() {
  const fetchProducts = async (sortBy: string) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://nidas-be-jnwz.onrender.com'}/products?status=active`);
      ...
      const filtered = (data.data || []).filter((item: Product) => {
        ...
        if (item.name.toLowerCase().includes('sl 72') || item.name.toLowerCase().includes('sl72'))
          return true;
  ...
  return (
    <ShoesPageLayout
      pageTitle="ADIDAS SL 72"
      pageDescription="Discover the classic SL 72 collection"
      category="SL 72"
      fetchProducts={fetchProducts}
      emptyMessage="No SL 72 products found"
      tabs={TABS}
    />
  );
}
