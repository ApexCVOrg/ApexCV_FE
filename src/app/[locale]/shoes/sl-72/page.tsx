...
export default function SL72Page() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const desiredPath = ['Shoes', 'Adidas', 'SL 72'];
        ...
        const filtered = (data.data || []).filter((item: any) => {
          ...
          if (item.categories && Array.isArray(item.categories)) {
            ...
            if (categoryNames.includes('sl 72') || categoryNames.includes('sl72')) return true;
          }
          
          // Cách 4: Kiểm tra trong name hoặc description
          if (item.name.toLowerCase().includes('sl 72') || item.name.toLowerCase().includes('sl72')) return true;
      ...
      <Box sx={{ position: 'relative', zIndex: 2 }}>
        ...
        <Box sx={{ px: { xs: 2, md: 6 }, pt: 4, pb: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 1 }}>
            ...
            <Typography component="span" sx={{ color: '#000', mx: 0.5 }}>/</Typography>
            <Typography component="span" sx={{ color: 'text.primary', fontWeight: 500, textDecoration: 'underline', textUnderlineOffset: '4px', fontSize: '1rem' }}>SL 72</Typography>
          </Box>
          ...
          <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-start', alignItems: 'flex-end', pb: 2 }}>
            {TABS.map(tab => (
              <Link key={tab.value} href={`/shoes/${tab.value}`} style={{ textDecoration: 'none', flex: 1 }}>
                <Box sx={{
                  border: 0,
                  borderBottom: tab.value === 'sl-72' ? '4px solid #000' : 'none',
                  bgcolor: tab.value === 'sl-72' ? '#000' : '#fff',
                  color: tab.value === 'sl-72' ? '#fff' : '#111',
                  fontWeight: tab.value === 'sl-72' ? 700 : 500,
                  textAlign: 'center',
                  cursor: 'pointer',
                  transition: 'all 0.2s',
                  p: 0,
                  ...
                  '&:hover': {
                    ...
                    bgcolor: tab.value === 'sl-72' ? '#000' : '#f5f5f5',
                    color: '#000',
                    fontWeight: 700,
                  ...
                  <Typography sx={{ py: 2, fontWeight: tab.value === 'sl-72' ? 700 : 600, fontSize: '1.1rem', letterSpacing: 1, bgcolor: tab.value === 'sl-72' ? '#000' : '#fff', color: tab.value === 'sl-72' ? '#fff' : '#111', textTransform: 'uppercase' }}>{tab.label}</Typography>
                </Box>
              </Link>
            ))}
          </Box>
