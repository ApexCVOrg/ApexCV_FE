          ...
          <List dense disablePadding>
            {allNewShoes.map((item) => (
              <ListItem key={item.name} disableGutters sx={{ py: 0.2 }}>
                <Link href={item.route} style={{ textDecoration: 'none', color: 'inherit', width: '100%' }}>
                  <ListItemText primary={item.name} primaryTypographyProps={{ fontWeight: 400, fontSize: 15, sx: { cursor: 'pointer' } }} />
                </Link>
              </ListItem>
            ))}
          </List>
        </Box>
        <Box sx={{ pl: 3, flex: 1, display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 3, alignItems: 'flex-start', justifyItems: 'center' }}>
          {featured.slice(0, 2).map((shoe) => (
            <Link key={shoe.name} href={shoe.route} style={{ textDecoration: 'none', color: 'inherit' }}>
              <Box>
                <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 1, textTransform: 'uppercase', textAlign: 'center', cursor: 'pointer' }}>{shoe.name}</Typography>
                <Box sx={{ width: 260, height: 180, bgcolor: '#f7f7f7', borderRadius: 2, overflow: 'hidden', mb: 1, p: 0, m: 0 }}>
                  <img src={shoe.img} alt={shoe.name} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                </Box>
              </Box>
            </Link>
          ))}
